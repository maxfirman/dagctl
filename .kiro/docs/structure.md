...
### `src/main.rs`
Defines the CLI structure with clap derive macros. Top-level commands: `get`, `events`, `logs`, `schema`, `debug`, `completion`, `self`. Parses args, resolves auth, dispatches to command handlers. Creates a tokio runtime for async commands.
