...
Query Dagster Cloud Insights metrics over time — credits, compute duration, materializations, and custom metrics.
...
**Metric short names:** dagster-credits, compute-duration, run-duration, materializations, observations, step-failures, step-retries, failures, snowflake-credits. You can also pass raw internal names (e.g. `--metric __meta_num_rows`). Use `dagctl get insights-metrics` to discover all available metrics.
